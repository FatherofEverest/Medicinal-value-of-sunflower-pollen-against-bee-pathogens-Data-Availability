...
ylabel<-expression(italic(Nosema)~infection~("cells*"~0.02~mu*L^-1)) #"*" for no-space
xlabel<- "Time (d)"
x.ticklabs<- c("10", "15")
Pollen.names<- c("No Pollen", "Buckwheat", "Sunflower")

plot0<- ggplot(Lsmeans.df, aes(x=Time, y=btmean, shape = Treatment, color = Treatment)) + 
  
  geom_point(position=position_dodge(0.5), stat="identity", size = 3 ) +
  geom_errorbar(aes(ymin=btlo, ymax=bthi),
                size=1, width=0.1,  # Width of the error bars
...
p1<- plot0 + ylab (ylabel) + xlab (xlabel) +
  ...
  scale_shape_discrete(name = NULL, labels = Pollen.names) +
  scale_color_discrete(name = NULL, labels = Pollen.names)
p1
...
#Bonus 2: Add raw data
Nosema.inf<- p.let + geom_point(data = Dm, aes(x= Time, y = Count), position=position_dodge(0.5), 
                   size = 1, alpha = 0.4) +
  ylim(0, 1.05*max(Dm$Count))
Nosema.inf
#ggsave("Nosema.sunflower.v1.reorder.pdf")
