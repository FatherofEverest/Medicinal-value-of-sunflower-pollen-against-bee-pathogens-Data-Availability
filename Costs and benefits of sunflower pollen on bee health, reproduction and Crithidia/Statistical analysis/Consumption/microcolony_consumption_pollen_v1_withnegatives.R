...
Anova(M03)
summary(M03)
#M03<-lme (PRate ~ Infection + Pollen + Pollen:Time , data=Data, random= ~1|Colony/Microcolony/Time, method="ML") 

#here is the same model with lme4 
Data<-droplevels(Data)
lmer03<- lmer(PRate ~ Infection + Pollen + Pollen:Time + (1|Colony/Microcolony/Time), data=Data)

#and starting from scratch in lme4
library(lme4)
AltModel<-lmer(PRate ~ Infection * Pollen * Time + (1|Colony/Microcolony/Time), data=Data)
#oops, model gave an error
#try leaving out the "Time" term since we use that as fixed effect
Alt1<-lmer(PRate ~ Infection * Pollen * Time + (1|Colony/Microcolony), data=Data)
summary(Alt1)
Alt1ML<- lmer(PRate ~ Infection * Pollen * Time + (1|Colony/Microcolony), data=Data, REML=FALSE)
Anova(Alt1)
AICtab(Alt1, M0) #model with full nesting wins big
...
#I also prefer to have "time" nested within Colony=inoculation date
#because in reality the 7d measurement is on different days (different temp, humidity)
