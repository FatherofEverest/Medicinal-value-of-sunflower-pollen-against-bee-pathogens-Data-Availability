...
Data<-read.csv("C:/Users/Evan/Dropbox/Jess Sum15/Pollen consumption trials/updated survival.csv")#rename some columns
View(Data)
...
nbinom01<-glmmadmb(totcrith ~ Pollen  + wingmm + (1 |Colony/ Microcol/BeeID), 
                  family = "nbinom",
                  data=Winged)  #got an error again, let's try to remove wing size
nbinom001<-glmmadmb(totcrith ~ Pollen  +  (1 |Colony/ Microcol/BeeID), 
                   family = "nbinom",
                   data=Data) #finally no error!
nbinom01<-glmmadmb(totcrith ~ Pollen  + wingmm + (1 |Colony/ Microcol/BeeID), 
                   family = "nbinom",
                   data=Winged, debug=TRUE)  #figuring out errors...
#"Parameters were estimated, but standard errors were not: 
#the most likely problem is that the curvature at MLE was zero or negative"
#so just go with the simpler model

Anova(nbinom001)
summary(nbinom001)
library(bbmle)
AICtab (nbinom01,nbinom001 ) #this doesn't work, maybe because of errors in model fitting?
AIC(nbinom01) 
AIC(nbinom001) #they are same
summary(nbinom1)
...
#nlme package author Douglas Bates says warnings indicate over-specified model
#https://stat.ethz.ch/pipermail/r-sig-mixed-models/2008q2/000912.html


#Compare fits with 2 packages
logLik(M1)
#compare to glmmadmb fit:
logLik(CountFinal) #slightly better
AICtab (M1, CountFinal)
#glmmadmb better by 6 aic units
