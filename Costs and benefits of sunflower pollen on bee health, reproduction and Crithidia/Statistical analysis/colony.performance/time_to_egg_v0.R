...
Bees<-read.csv("updated_survival_v2_fix_56.4.csv")
...
length(Measured$Microcolony) #lost 10 microcolonies-- too many!
...
Nested.Plus<-coxme(Surv(Death_Days, Dead) ~ 
                     Infection*Pollen +  dimorph + (1|Inoculation.date/Microcolony) , data=D1 )
library(car)
summary(Nested.Plus)
Anova(Nested.Plus)
...
NP1a<-coxme(Surv(Death_Days, Dead) ~ 
              Infection*Pollen +  (1|Inoculation.date/Microcolony) , data=Data )
summary(NP1a)
...
plot(
  frame=FALSE,
  Bee.Survival.Treatment,
  conf.int = FALSE,
  xlab=expression(bold("Time (d)")), 
  ylab=expression(bold("Proportion Without Egg")), 
  xlim = c(0,45), 
  ylim = c(0.0,1.02) ,
...
legend("topright",bty="n", #removes box around legend
