...
Anova(fitNB)
#drop interaction
P1<-update(fitPoiss, ~. - Infection:Pollen)
Anova(P1)

###CHOSE THIS AS FINAL MODEL########
NB1<-update(fitNB, ~. - Infection:Pollen)
summary(NB1)
Anova(NB1)
library(lsmeans)
lsmeans(NB1, ~Pollen, type="response")
...
#Let's plot this
library(lsmeans)
testLS<-lsmeans(P1, ~Pollen)
plot(testLS)
str(testLS) #note log link function
test2<-lsmeans(P1, ~Pollen, type="response")
test2
test3<-lsmeans(NB1, ~Pollen, type="response")
str(test3) #note log link function
test3
...
LCOUNTS<-ddply(Perf03,~Infection*Pollen, 
               summarise,numlarvae=mean(larvnum),
               meanmass=mean(larvwt, na.rm=TRUE))
LCOUNTS #these are the arithmetic means, similar to margmeans2 
# in the lsmeans output
#I will exponentiate manually to check this
...
Lsmeans.df<-margmeans2
str(Lsmeans.df)
View(Lsmeans.df)
#label for x axis
pollen.axis<-c( "Buckwheat", "Sunflower")
#########option 1: use symmetric standard error#########
...
#########option 2 (preferred): #################
#use mean +-SE manually exponentiated to response scale#
...
pdf("LARVALNUMBER.v2.manualSE.pdf",height=6, width=5, paper='special') 
fig1
dev.off()

#for eps
setEPS()
postscript("LARVALNUMBER.v2.manualSE.eps", horizontal = FALSE, onefile = FALSE, 
...
LSM
back.lsmeans(lsm=LSM, transform = c("log"), add=0 )
exp(0.2) #not matching the manual calculation...
log(0.2)
LS1<-summary(LSM)
