...
p<-ggplot(Lsmeans.df, aes(x = Pollen, y = Count)) + 
  ...
  geom_point(data = Perf03, aes( x = Pollen, y = larvnum), position = "jitter", 
...
p
#Add text
Lfig<- p + geom_text(aes(y = bthi + 0.4 * max(bthi), label = .group),
                          size = 8)
Lfig

##Export
# setwd("C:/Users/Evan/Google Drive/Project_Sunflower/Microcolony/jess stats/colony.performance")
# ggsave("LARVALNUMBER.v3.TMB.pdf", height = 5, width =5)
