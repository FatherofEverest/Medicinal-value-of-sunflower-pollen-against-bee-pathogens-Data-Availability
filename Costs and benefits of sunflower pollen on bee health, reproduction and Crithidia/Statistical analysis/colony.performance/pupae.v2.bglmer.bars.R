...
#edit 2017.07.01: change order of post hoc letters
#edited 2017.06.11: Bar chart
...
p
#Add text
Pfig<-p + geom_text(aes(y = bthi + 0.1 * max(bthi), label = .group),
                    size = 8)
Pfig

##Export
# setwd("C:/Users/Evan/Google Drive/Project_Sunflower/Microcolony/jess stats/colony.performance")
# ggsave("pupae.v4.bglmer.reletter.pdf", height = 5, width =5)

##Consolidate metrics
#A. Number of eggs
Panel.2a
#B. Number of larvae
#("Larvalnumber.v3.TMB.R"... or latest larval number script)
Lnum
#C. Mean larval mass
#("Larvalmass.v1.lme4.R"... check for latest version)
P.lmass
#D. Proportion with pupae
Pfig

#Clean up:remove redundant axes
A.neat<-Panel.2a #+ theme(legend.position = "none") ##keep legend, this one is unique (infection)
B.neat<- Lnum + theme(axis.title.x = element_blank())
C.neat<- P.lmass + theme(axis.title.x = element_blank())
D.neat<-Pfig + theme(axis.title.x = element_blank())
D.neat
#Arrange and plot... MOve old panel A to panel D
MC.all<-plot_grid(B.neat, C.neat, D.neat, A.neat,
          nrow=1, labels = LETTERS,
          label_size = rel(20))
MC.all
setwd("C:/Users/Evan/Google Drive/Project_Sunflower/Microcolony/jess stats/colony.performance")
#ggsave("MC.performance.4panel.v3.reletter.pdf", height = 4, width = 10)

#Save to main figures folder
