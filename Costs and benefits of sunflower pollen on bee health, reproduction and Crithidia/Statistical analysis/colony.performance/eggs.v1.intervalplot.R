...
plot0<- ggplot(margmeans, aes(x=Pollen, y=btmean, shape = Infection, color=Infection)) +
  geom_point(data = Perf03, aes ( x = Pollen, y = eggnum), 
             position=position_dodge(0.5),
             alpha = 0.3) +
  geom_pointrange(aes(y=btmean, ymin=btlo, ymax=bthi),
                  position=position_dodge(0.5)) +
   ylab("Number of eggs") + xlab("Pollen")+
   scale_shape_discrete(name=Cbombi, #use object created above
                        labels=c("Uninfected", "Infected")) +  #Legend: full treatment names
  scale_color_manual(name=Cbombi, #use object created above
                       values = c("black", "red"),
                     labels=c("Uninfected", "Infected")) +  
  scale_x_discrete(labels=pollen.axis.short)
plot0

#give directory for output file
setwd("C:/Users/Evan/Google Drive/Project_Sunflower/Microcolony/jess stats/colony.performance")
ggsave("eggfigure.v2.intervalplot.pdf", height = 4, width = 4)
