...
My.Lsmeans
Lets<-cld(My.Lsmeans, Letters = letters)
...
p<-ggplot(Lsmeans.df, aes(x = Pollen, y = Count)) + 
  ...
  geom_point(data = D, aes( x = Pollen, y = Count), position = "jitter", 
...
p
#Add text
Panel.1b <- p + geom_text(aes(y = bthi + 0.2 * max(bthi), label = .group),
              size = 8)
Panel.1b
#ggsave("Exp2_v2.TMB.pdf", height = 5, width =5)

#Example of plot_grid
Panel.AB<-plot_grid(Panel.1a, Panel.1b)
