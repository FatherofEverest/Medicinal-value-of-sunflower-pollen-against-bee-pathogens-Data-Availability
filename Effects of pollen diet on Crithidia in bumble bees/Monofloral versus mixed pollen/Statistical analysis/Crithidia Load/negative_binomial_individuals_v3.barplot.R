...
#update 2017.07.01: reorder letters for cld
##Updated 2017.06.03: Use glmmTMB instead of glmmADMB
#update 2017.06.11 : barplot with color scheme
...
Data1<-dplyr::rename(Data1, Pollen = Treatment)
names(Data1)
Data2<-na.omit(Data1)
###### Refit with glmmTMB ###
...
Lets<-cld(lsm.TMB, Letters = letters, sort = FALSE)
Lets
...
p
#Add text
Panel.1a<-p + geom_text(aes(y = bthi + 0.1 * max(bthi), label = .group),
              size = 8)
Panel.1a
